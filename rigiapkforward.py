...
def handle_messages(message):
    ...
    if str(message.from_user.id).startswith('5252650067'):
        if message.text.startswith('/set_channel'):
            try:
                target_channel = "-100" + str(message.text.split()[1])  # Extract channel ID from the message
            ...
            bot.reply_to(message, f"Target channel: {target_channel}\nImage forwarding: {'ON' if forward_images else 'OFF'}")
            ...
            bot.reply_to(message, "Invalid command. Please use one of the available commands.")
